...
void main() {
  ...
  void expectValid(ExportResult result) {
    ...
    expect(
      file.lengthSync(),
      greaterThan(0),
      reason: 'output should be non-empty',
    );
  }
  ...
  testWidgets('image slideshow with audio muxes a soundtrack', (tester) async {
    final silentBaseline = await engine.exportVideo(
      imagePath: redImage,
      mediaPaths: [redImage, blueImage],
      imagePaths: [redImage, blueImage],
    ...
    expect(
      _looksLikeAudioWasMuxed(result.outputPath, silentBaseline.outputPath),
      isTrue,
  ...
  testWidgets('single video source with audio exports', (tester) async {
    ...
    expect(_looksLikeAudioWasMuxed(result.outputPath, clip.outputPath), isTrue);
  });
}
...
bool _looksLikeAudioWasMuxed(String path, String silentBaselinePath) {
  // video_player has no audio-track API, so compare against a silent baseline
  // generated by the same native engine. AAC output for these fixtures adds
  // several KB even when the tone is encoded very compactly.
  final size = File(path).lengthSync();
  final baselineSize = File(silentBaselinePath).lengthSync();
  return size > baselineSize + 4 * 1024;
}
...
Future<String> _writeSineWav(
  ...
  for (var i = 0; i < sampleCount; i++) {
    final value = (math.sin(2 * math.pi * 440 * i / sampleRate) * 12000)
        .toInt();
