...
class WatermarkController extends Notifier<WatermarkState> {
  ...
  void setOverlayWindow(int index, double start, double end) {
    ...
    final next = [...state.overlays];
    next[index] = next[index].copyWith(start: clampedStart, end: clampedEnd);
    state = state.copyWith(overlays: next);
  }
  ...
  Future<engine.ExportResult?> export() async {
    ...
    final overlayPaths = await _mediaStore.materializeMediaPaths(
      state.overlays.map((o) => o.path).toList(),
    );
    ...
    try {
      ...
      debugPrint(
        '[Watermark] export: FELL BACK to exportReel (no exportWatermark)',
      );
